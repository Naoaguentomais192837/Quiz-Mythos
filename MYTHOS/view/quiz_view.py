...
class QuizView(ft.View):
    def __init__(self, page, go_to, state=None):
        ...
        # pergunta: AGORA COM COR PRETA EXPLÍCITA (design original)
        self.question_text = ft.Text(
            "", 
            size=18, 
            weight=ft.FontWeight.BOLD, 
            selectable=False, 
            color=ft.Colors.BLACK # CORREÇÃO DE DESIGN
        )
        ...
        top_row = ft.Row(
            [
                ...
                ft.IconButton(icon=ft.Icons.ARROW_BACK_IOS, on_click=lambda _: self.go_to("menu"), icon_color=ft.Colors.BLACK),
    ...
    def _reset_buttons(self):
        """Restaura os botões ao estado neutro (branco)."""
        for btn in self.option_buttons:
            # CORREÇÃO ESSENCIAL DE DESIGN: Força o branco em vez de None
    ...
    def _load_question(self):
        ...
        if self.index >= len(self.questions):
            ...
            self.question_text.color = ft.Colors.BLACK 
            for btn in self.option_buttons:
                btn.text = "-"
                btn.bgcolor = None
                btn.color = ft.Colors.BLACK
        ...
        self._reset_buttons()
        self.locked = False
        # self.page.update() chamado dentro de _reset_buttons()
    ...
    def _handle_answer(self, chosen):
        ...
        # colore resposta
        if chosen == correct:
            self.option_buttons[chosen].bgcolor = ft.Colors.GREEN_700
            self.option_buttons[chosen].color = ft.Colors.WHITE
            self.state["correct_answers"] = self.state.get("correct_answers", 0) + 1
        else:
            self.option_buttons[chosen].bgcolor = ft.Colors.RED_700
            self.option_buttons[chosen].color = ft.Colors.WHITE
            self.option_buttons[correct].bgcolor = ft.Colors.GREEN_700
            self.option_buttons[correct].color = ft.Colors.WHITE
